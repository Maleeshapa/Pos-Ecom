...
for (const item of cartItems) {
  ...
  // Find all stock entries for the product with available quantity
  const availableStocks = await EcomStock.findAll({
      where: {
          productId: item.productId,
          stockQTY: { [Op.gt]: 0 } // Only stocks with quantity > 0
      },
      order: [['stockQTY', 'DESC']] // Use the one with the most stock first
  });

  for (const stock of availableStocks) {
      if (remainingQty <= 0) break;

      const deductionQty = Math.min(remainingQty, stock.stockQTY);
      await stock.update({ stockQTY: stock.stockQTY - deductionQty });

      remainingQty -= deductionQty;
  }

  // If after all deductions, there’s still remaining quantity, it's insufficient
  if (remainingQty > 0) {
      await t.rollback();
      return res.status(400).json({
          success: false,
          message: `Insufficient stock available for product ${item.productId}`
      });
  }
}
